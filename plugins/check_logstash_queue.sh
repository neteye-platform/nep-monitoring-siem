...
one of 'percentage, MB, GB' (default ${THRESHOLD_FORMAT})"
  ...
  echo -e "\t--logstash-version (int|str): Major Logstash version (supported options: 6, 7, 8, 9, 'auto'). 'auto' tries to autodetect the version (default: 'autodetect')"
  ...
  echo -e "\t--pipeline (str): set pipeline name to check"
}
...
case "${LOGSTASH_VERSION}" in
  ...
  9)
    ...
    ;;
  *)
    echo "CHECK UNKNOWN - Unsupported logstash version '${LOGSTASH_VERSION}'. Version supported 6.x, 7.x, 8.x and 9.x"
    exit ${UNKNOWN}
esac
