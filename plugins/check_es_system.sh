...
DEFAULT_TOTAL_SHARD_PER_NODE=1000              # Default values for Elasticsearch
...
Options:
   ...
   *  -t Type of check (disk, mem, cpu, status, readonly, jthreads, tps, master, maxshards)
...
Threshold format for 'disk', 'mem','cpu' and 'maxshards': int (for percent), defaults to 80 (warn) and 95 (crit)
...
Requirements: curl, expr and one of $(
    IFS=,
    echo "${parsers[*]}"
  )"
  exit $STATE_UNKNOWN
...
authlogic() {
  if [[ -z $user ]] && [[ -z $pass ]]; then
    echo "ES SYSTEM UNKNOWN - Authentication required but missing username and password"
    exit $STATE_UNKNOWN
  elif [[ -n $user ]] && [[ -z $pass ]]; then
    echo "ES SYSTEM UNKNOWN - Authentication required but missing password"
    exit $STATE_UNKNOWN
  elif [[ -n $pass ]] && [[ -z $user ]]; then
    echo "ES SYSTEM UNKNOWN - Missing username"
    exit $STATE_UNKNOWN
  fi
}
...
authlogic_cert() {
  if [[ -z $cert ]] && [[ -z $key ]]; then
    echo "ES SYSTEM UNKNOWN - Authentication required but missing cert and key"
    exit $STATE_UNKNOWN
  elif [[ -n $cert ]] && [[ -z $key ]]; then
    echo "ES SYSTEM UNKNOWN - Authentication required but missing key"
    exit $STATE_UNKNOWN
  elif [[ -n $key ]] && [[ -z $cert ]]; then
    echo "ES SYSTEM UNKNOWN - Missing cert"
    exit $STATE_UNKNOWN
  fi
}

retrive_version() {
  es_version=$(echo $1 | json_parse -x version -x number | tr -d '"')
}
...
unitcalc() {
  ...
  if [[ -n $unit ]]; then
    case $unit in
    K)
      availsize=$(expr $available / 1024)
      outputsize=$(expr ${size} / 1024)
      ;;
    M)
      availsize=$(expr $available / 1024 / 1024)
      outputsize=$(expr ${size} / 1024 / 1024)
      ;;
    G)
      availsize=$(expr $available / 1024 / 1024 / 1024)
      outputsize=$(expr ${size} / 1024 / 1024 / 1024)
      ;;
    esac
    if [[ -n $warning ]]; then
    ...
    echo "UNKNOWN - Shouldnt exit here. No units given"
    exit $STATE_UNKNOWN
  fi
}

thresholdlogic() {
  if [ -n $warning ] && [ -z $critical ]; then
    echo "UNKNOWN - Define both warning and critical thresholds"
    exit $STATE_UNKNOWN
  fi
  if [ -n $critical ] && [ -z $warning ]; then
    echo "UNKNOWN - Define both warning and critical thresholds"
    exit $STATE_UNKNOWN
  fi
}
...
json_parse() {
  json_parse_usage() {
    echo "$0: [-r] [-q] [-c] [-a] -x arg1 -x arg2 ..." 1>&2
    exit
  }

  local OPTIND opt r q c a x
  while getopts ":rqcax:" opt; do
    case "${opt}" in
    r) raw=1 ;;
...
while getopts "H:LP:SE:K:u:p:d:o:i:w:c:t:m:e:X:" Input; do
  case ${Input} in
  H) host=${OPTARG} ;;
  L) local=true ;;
  P) port=${OPTARG} ;;
...
if [ -z ${parser} ]; then
  ...
  if [ -z "${parser}" ]; then
    echo "UNKNOWN: No JSON parser found. Either one of the following is required: $(
      IFS=,
      echo "${parsers[*]}"
    )"
    exit ${STATE_UNKNOWN}
...
master) # Check Cluster Master
  ...
  if [[ -n $cert ]] || [[ -n $(echo $esstatus | grep -i authentication) ]]; then
    ...
    if [[ $masterrc -eq 7 ]]; then
      ...
      echo "ES SYSTEM CRITICAL - server did not respond within ${max_time} seconds"
      exit $STATE_CRITICAL
    elif [[ "$masterrc" =~ "503 Service Unavailable" ]]; then
      echo "ES SYSTEM CRITICAL - Elasticsearch not available: ${host}:${port} return error 503"
      exit $STATE_CRITICAL
    elif [[ -n $(echo $esstatus | grep -i "unable to authenticate") ]]; then
      echo "ES SYSTEM CRITICAL - Unable to authenticate user $user for REST request"
...
maxshards) # Check Max Shards Setting
  getstatus

  if [[ -z $user ]] && [[ -z $cert ]]; then
    # Without authentication
    cluster_settings=$(curl -k -s --max-time ${max_time} ${httpscheme}://${host}:${port}/_cluster/settings)
    cluster_settingsrc=$?
    if [[ $cluster_settingsrc -eq 7 ]]; then
      echo "ES SYSTEM CRITICAL - Failed to connect to ${host} port ${port}: Connection refused"
      exit $STATE_CRITICAL
    elif [[ $cluster_settingsrc -eq 28 ]]; then
      echo "ES SYSTEM CRITICAL - server did not respond within ${max_time} seconds"
      exit $STATE_CRITICAL
    fi

    retrive_version "$(curl -k -s --max-time ${max_time} ${httpscheme}://${host}:${port}/)"
    ## Different path for 7.15 and 7.17
    if [ "$(printf '%s\n' "7.16.0" "$es_version" | sort -V | head -n1)" = "7.16.0" ]; then
      PATH_NODES_STC="_nodes/stats/indices/shard_stats?filter_path=nodes.*.indices.shard_stats.total_count"
    else
      PATH_NODES_STC="_nodes/stats/indices/shards?filter_path=nodes.*.indices.shards.total_count"
    fi
    node_shards_count=$(curl -k -s --max-time ${max_time} ${httpscheme}://${host}:${port}/${PATH_NODES_STC})
  fi

  if [[ -n $user ]] || [[ -n $(echo $esstatus | grep -i authentication) ]]; then
    # Authentication required
    authlogic
    cluster_settings=$(curl -k -s --max-time ${max_time} --basic -u ${user}:${pass} ${httpscheme}://${host}:${port}/_cluster/settings)
    cluster_settingsrc=$?
    if [[ $cluster_settingsrc -eq 7 ]]; then
      echo "ES SYSTEM CRITICAL - Failed to connect to ${host} port ${port}: Connection refused"
      exit $STATE_CRITICAL
    ...
    retrive_version "$(curl -k -s --max-time ${max_time} --basic -u ${user}:${pass} ${httpscheme}://${host}:${port})"
    ## Different path for 7.15 and 7.17
    if [ "$(printf '%s\n' "7.16.0" "$es_version" | sort -V | head -n1)" = "7.16.0" ]; then
      PATH_NODES_STC="_nodes/stats/indices/shard_stats?filter_path=nodes.*.indices.shard_stats.total_count"
    else
    ...
    node_shards_count=$(curl -k -s --max-time ${max_time} --basic -u ${user}:${pass} ${httpscheme}://${host}:${port}/${PATH_NODES_STC})
  fi

  if [[ -n $cert ]] || [[ -n $(echo $esstatus | grep -i authentication) ]]; then
    # Authentication with certificate
    authlogic_cert
    cluster_settings=$(curl -k -s --max-time ${max_time} -E ${cert} --key ${key} ${httpscheme}://${host}:${port}/_cluster/settings)
    cluster_settingsrc=$?
    if [[ $cluster_settingsrc -eq 7 ]]; then
      echo "ES SYSTEM CRITICAL - Failed to connect to ${host} port ${port}: Connection refused"
      exit $STATE_CRITICAL
      ...
      echo "ES SYSTEM CRITICAL - server did not respond within ${max_time} seconds"
      exit $STATE_CRITICAL
    elif [[ "$cluster_settings" =~ "503 Service Unavailable" ]]; then
      echo "ES SYSTEM CRITICAL - Elasticsearch not available: ${host}:${port} return error 503"
      exit $STATE_CRITICAL
    elif [[ -n $(echo "$cluster_settings" | grep -i "unable to authenticate") ]]; then
      echo "ES SYSTEM CRITICAL - Unable to authenticate user $user for REST request"
      exit $STATE_CRITICAL
    elif [[ -n $(echo "$cluster_settings" | grep -i "unauthorized") ]]; then
      echo "ES SYSTEM CRITICAL - User $user is unauthorized"
      exit $STATE_CRITICAL
    fi

    retrive_version "$(curl -k -s --max-time ${max_time} -E ${cert} --key ${key} ${httpscheme}://${host}:${port}/)"
    ## Different path for 7.15 and 7.17
    if [ "$(printf '%s\n' "7.16.0" "$es_version" | sort -V | head -n1)" = "7.16.0" ]; then
      PATH_NODES_STC="_nodes/stats/indices/shard_stats?filter_path=nodes.*.name,nodes.*.indices.shard_stats.total_count"
    else
      PATH_NODES_STC="_nodes/stats/indices/shards?filter_path=nodes.*.name,nodes.*.indices.shards.total_count"
    fi
    node_shards_count=$(curl -k -s --max-time ${max_time} -E ${cert} --key ${key} ${httpscheme}://${host}:${port}/${PATH_NODES_STC})
    node_shards_countrc=$?
  fi

  # Catch empty reply from server (typically happens when ssl port used with http connection)
  if [[ -z $cluster_settings ]] || [[ $cluster_settings = '' ]]; then
    echo "ES SYSTEM UNKNOWN - Empty reply from server (verify ssl settings)"
    exit $STATE_UNKNOWN
  fi

  TOTAL_SHARDS_PER_NODE=$(echo "$cluster_settings" | jq -c '.persistent.cluster.max_shards_per_node')

  if [[ $TOTAL_SHARDS_PER_NODE != null ]]; then
    CLUSER_TOTAL_SHARDS_PER_NODE=$(echo "$TOTAL_SHARDS_PER_NODE" | jq -c 'tonumber')
  else
    #echo "No explicit setting for total_shards_per_node was found: we use a default value $DEFAULT_TOTAL_SHARD_PER_NODE"
    CLUSER_TOTAL_SHARDS_PER_NODE=$DEFAULT_TOTAL_SHARD_PER_NODE
  fi

  ## Different path for 7.15 and 7.17
  if [ "$(printf '%s\n' "7.16.0" "$es_version" | sort -V | head -n1)" = "7.16.0" ]; then
    jqShardPath=".indices.shard_stats.total_count"
  else
    jqShardPath=".indices.shards.total_count"
  fi

  declare -A nodeShard_Map=()

  nodeName=""
  nodeShards=""
  IDS=$(echo "$node_shards_count" | jq '.nodes | keys[]')

  for idNode in $IDS; do
    nodeName=$(echo "$node_shards_count" | jq --raw-output '.nodes.'$idNode' | .name')
    ## Es: 7.17.5
    nodeShards=$(echo "$node_shards_count" | jq '.nodes.'$idNode' | '$jqShardPath' | tonumber')
    ##
    #nodeShards=$(echo "$node_shards_count" | jq '.nodes.'$idNode' | '$jqShardPath' | tonumber')
    ##

    nodeShard_Map[$nodeName]=$nodeShards
  done

  if [[ ${#nodeShard_Map[@]} < 1 ]]; then
    echo "UNKNOWN - Shouldnt retrive total shard per node count"
    exit $STATE_UNKNOWN
  fi
  ...
  warningsize=$(expr $CLUSER_TOTAL_SHARDS_PER_NODE \* ${warning} / 100)
  criticalsize=$(expr $CLUSER_TOTAL_SHARDS_PER_NODE \* ${critical} / 100)

  # Handle tresholds
  thresholdlogic

  declare -A criticalNodes=()
  declare -A warnignNodes=()
  declare -A okNodes=()

  metrics=""

  for name in ${!nodeShard_Map[@]}; do
    if [[ ${nodeShard_Map[$name]} -ge $criticalsize ]]; then
      #echo "CRITICAL "
      criticalNodes[$name]=${nodeShard_Map[$name]}
    elif
      [[ ${nodeShard_Map[$name]} -ge $warningsize && ${nodeShard_Map[$name]} -lt $criticalsize ]]
    then
      #echo "WARNING "
      warnignNodes[$name]=${nodeShard_Map[$name]}
    else
      [[ ${nodeShard_Map[$name]} -lt $warningsize ]]
      #echo "OK "
      okNodes[$name]=${nodeShard_Map[$name]}
    fi
    metrics=$metrics" '$name'=${nodeShard_Map[$name]};${warningsize};${criticalsize};0;"
  done

  if [[ ${#criticalNodes[@]} -gt 0 ]]; then

    echo "ES SYSTEM CRITICAL - Cluster's number of shard exceeded the critical threshold of $criticalsize in a one or more nodes.<br>"

    for key in ${!criticalNodes[@]}; do
      echo $key ":" ${criticalNodes[$key]}
    done
    echo " |"$metrics
    ...
    [[ ${#warnignNodes[@]} -gt 0 ]]
  then
    echo "ES SYSTEM WARNING - Cluster's number of shard exceeded the warnign threshold of $warningsize in a one or more nodes.<br>"

    for key in ${!warnignNodes[@]}; do
      echo $key ":" ${warnignNodes[$key]}
    done
    echo " |"$metrics
    ...
    [[ ${#okNodes[@]} -gt 0 ]]
    echo "ES SYSTEM OK - Cluster's number of shard is ok in all nodes.<br>"

    for key in ${!okNodes[@]}; do
      echo $key ":" ${okNodes[$key]}
    done
    echo " |"$metrics
